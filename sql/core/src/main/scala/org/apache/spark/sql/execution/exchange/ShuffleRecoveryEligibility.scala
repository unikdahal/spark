...
private[sql] object ShuffleRecoveryEligibility {
  ...
  final class Analyzer(val ruleSet: RuleSet) {
    ...
    private[sql] def analyzeCompleted(qe: QueryExecution): Seq[Record] = {
      analyze(qe.executedPlan, qe.queryId.toString, AnalysisRuntime.from(qe))
    }
    ...
    private def analyzeCandidate(
      ...
      candidate.exchange.outputPartitioning match {
        case expression: Expression =>
          inspectExpressions(Seq(expression), candidate.path, flags, addFinding)
        case _ =>
      }
      ...
      // Semantic precedence is independent of wrapper/traversal order so a stable root miss can be
      // aggregated across plan-shape changes without a wrapper-level miss hiding a deeper cause.
  ...
  final class Listener(
    ...
    override def onSuccess(
      ...
      val records = analyzer.analyzeCompleted(qe)
